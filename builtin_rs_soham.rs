...
pub fn test(value: &Value, re: &Value) -> Result<Value> {
  match (value, re) {
    ...
    (Value::String(value), Value::String(re)) => {
      let re: Regex = re.replace(r"\\\\", r"\\").parse().map_err(|err| {
...
pub fn parse_time( 
    timestamp: &Value, 
    format: &Value, 
    _fallback: Option<&Value> 
) -> Result<Value> { 
    match (timestamp, format) { 
        (Value::Null, _) => Ok(Value::Null), 
        (Value::String(ts), Value::String(fmt)) => { 
            let mut ts_clean = ts.clone(); 
            if let Some(alpha_end) = ts_clean.rfind(|c: char| c.is_ascii_alphabetic()) { 
                let alpha_start = ts_clean[..=alpha_end] 
                    .rfind(|c: char| !c.is_ascii_alphabetic()) 
                    .map(|i| i + 1) 
                    .unwrap_or(0); 
                let (base, zone) = ts_clean.split_at(alpha_start); 
                let mapped = match zone { 
                    "CET" => "+01:00",  
                    other => other, 
                }; 
                ts_clean = format!("{}{}", base, mapped); 
            } 
            let chrono_fmt = preproces_format(fmt); 
            // — try strptime; if that fails, fall back to RFC3339 
            match DateTime::parse_from_str(&ts_clean, &chrono_fmt) { 
                Ok(dt) => { 
                    (dt.with_timezone(&Utc) - DateTime::UNIX_EPOCH) 
                        .to_std() 
                        .map(|d| d.as_secs_f64().into()) 
                        .map_err(|e| JsltError::InvalidInput(e.to_string())) 
                } 
                Err(_) => {  
                    match DateTime::parse_from_rfc3339(&ts_clean) { 
                        ...
                        Err(err) => 
                            ...
                            Err(JsltError::InvalidInput(err.to_string())), 
                    } 
                } 
            } 
        } 
        _ => Err(JsltError::InvalidInput( 
            "Bad arguments for parse-time".to_string() 
        )), 
    } 
} 

fn preproces_format(format: &String) -> String{
  let mut adjusted_format=format.clone();
  adjusted_format=adjusted_format.replace("yyyy","%Y");
  adjusted_format=adjusted_format.replace("yy", "%y");
  adjusted_format=adjusted_format.replace("dd","%d");
  adjusted_format=adjusted_format.replace("MM","%m");
  adjusted_format=adjusted_format.replace("HH","%H");
  adjusted_format=adjusted_format.replace("mm","%M");
  adjusted_format=adjusted_format.replace("ss","%S");
  adjusted_format=adjusted_format.replace("z","%Z");
...
pub fn format_time(timestamp: &Value, format: &Value, _timezone: Option<&Value>) -> Result<Value> {
  match (timestamp, format) {
    ...
    (Value::Number(timestamp), Value::String(format)) => {
      ...
      let adjusted_format=preproces_format(format);

      Ok(Value::String(timestamp.format(&adjusted_format).to_string()))
    }
    _ => Err(JsltError::InvalidInput(
