...
impl Transform for BooleanTransformer {
  fn transform_value(&self, _: Context<'_>, _: &Value) -> Result<Value> {
    trace!("▶ BooleanTransformer → {:?}", self.0);
...
impl Transform for NumberTransformer {
  fn transform_value(&self, _: Context<'_>, _: &Value) -> Result<Value> {
    trace!("▶ NumberTransformer → {:?}", self.0);
...
impl Transform for ScopeTransformer {
  fn transform_value(&self, context: Context<'_>, input: &Value) -> Result<Value> {
    trace!("▶ ScopeTransformer (delegating)");
...
impl Transform for StringTransformer {
  fn transform_value(&self, _: Context<'_>, _: &Value) -> Result<Value> {
    trace!("▶ StringTransformer → {:?}", self.0);
...
impl Transform for ValueTransformer {
  fn transform_value(&self, context: Context<'_>, input: &Value) -> Result<Value> {
    trace!("▶ ValueTransformer::transform_value; variant = {:?}", self);
    ...
    trace!("◀ ValueTransformer::transform_value; {:?} => {:?}", self, result);
    Ok(result)
  }
}
...
impl Transform for VariableTransformer {
  fn transform_value(&self, context: Context<'_>, _: &Value) -> Result<Value> {
    trace!("▶ VariableTransformer lookup `${}`", self.0);
    // let value = context.variables[&self.0].clone();
    // let value = context.local_variables

    let value = context.local_variables 

        .get(&self.0) 

        .or_else(|| context.variables.get(&self.0)) 

        .cloned()
        .ok_or_else(||JsltError::UnexpectedEnd)?; 

        // .ok_or_else(|| JsltError::UnexpectedEnd(self.0.clone()))?; 
      
    match &self.1 {
      Some(accessor) => 
      { trace!("  applying accessor on `${}`", self.0);
        accessor.transform_value(context, &value)
      }
      None => Ok(value),
    }
  }
