...
macro_rules! trace {
    ($fmt:expr $(, $arg:expr)*) => {
        println!("[TRACE] {}:{}: {}", file!(), line!(), format!($fmt $(, $arg)*));
    };
}
...
impl Jslt {
  ...
  where
    S: for<'de> Deserialize<'de> + Debug,
    T: Serialize + Debug,  
  {
    // change 1
    trace!("▶ Jslt::transform: raw input = {:?}", input);
    let json_input = serde_json::to_value(input)?;
    trace!("  → serde_json::to_value = {:?}", json_input);


    let value = self.transform_value(&json_input)?;
    trace!("  → transform_value = {:?}", value);


    // let value = self.transform_value(&serde_json::to_value(input)?)?;
     serde_json::from_value(value.clone()).map_err(Into::into)
    // trace!("  → serde_json::from_value = {:?}", result);
  ...
  pub fn transform_value(&self, input: &Value) -> Result<Value> {
    trace!("▶ Jslt::transform_value: input JSON = {}", input);
    let out=self
      .builder
      .transform_value(Cow::Borrowed(&self.context), input)?;
    trace!("  → builder.transform_value = {:?}", out);
...
impl FromStr for Jslt {
  ...
  fn from_str(value: &str) -> Result<Self> {
    // change 3
    trace!("▶ Jslt::from_str: script = \n{}", value);
    let mut pairs = JsltGrammar::parse(Rule::Jslt, value)
      .map_err(|e| {
        trace!("  ✖ JsltGrammar::parse error: {}", e);
        Box::new(e)
      })?;
    trace!("  → JsltGrammar::parse produced {} pairs", pairs.clone().count());
    let builder = ExprTransformer::from_pairs(&mut pairs)?;
    trace!("  → ExprTransformer::from_pairs → {:#?}", builder);
    let context = JsltContext::default();
    trace!("  → JsltContext::default = {:?}", context);
    let jslt = Jslt { builder, context };
    trace!("◀ Jslt::from_str ⇒ Jslt ready");
    Ok(jslt)
